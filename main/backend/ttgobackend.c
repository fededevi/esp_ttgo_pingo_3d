...
void _ttgoDrawPixel( Texture * f, Vec2i pos, Pixel color, float illumination) {
    float r = color.g * illumination * 3;
    float g = color.g * illumination * 3+0.33;
    float b = color.g * illumination * 3 +0.33+0.33;
    copyBuffer[ping%2][pos.x][ pos.y ] = color565(r,g,b);
}
...
void ttgoBackendInit( TTGOBackend * this,  Vec2i size) {
    ...
    this->backend.drawPixel = &_ttgoDrawPixel;
